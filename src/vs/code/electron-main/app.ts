...
export class CodeApplication extends Disposable {
	...
	private registerListeners(): void {
		...
		// [FORK] Handle Agent Manager window open request from renderer
		validatedIpcMain.handle('vscode:openAgentManager', async () => {
			this.openAgentManagerWindow();
		});
	...
	// [FORK] Agent Manager dedicated BrowserWindow
	// Opens either the Next.js companion app or the bundled standalone HTML fallback
	private openAgentManagerWindow(): void {
		...
		const agentWindow = new BrowserWindow({
			...
			minWidth: 600,
			minHeight: 500,
			backgroundColor: '#1e1e1e', // [FORK] Match --wb-bg token
			titleBarStyle: 'hidden',    // [FORK] Custom chrome via HTML titlebar
			frame: false,
			title: 'Agent Manager',
			show: false,
			trafficLightPosition: { x: 12, y: 10 }, // macOS traffic lights
			webPreferences: {
				nodeIntegration: false,
				contextIsolation: true,
				// [FORK] Preload will expose __AGENT_BACKEND__ to window context
			},
		});

		// [FORK] Load Next.js companion if running, else fall back to standalone HTML
		const agentUrl = process.env['AGENT_MANAGER_URL'] ?? null;
		if (agentUrl) {
			agentWindow.loadURL(agentUrl);
		} else {
			// eslint-disable-next-line no-restricted-globals
			const htmlPath = path.join(__dirname, '../../../../fork/agent/index.html');
			agentWindow.loadFile(htmlPath);
		}

		agentWindow.once('ready-to-show', () => agentWindow.show());
		this.logService.info('[FORK] Agent Manager window opened');
	}
}
